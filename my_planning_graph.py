...
class PlanningGraph():
    ...
    def add_action_level(self, level):
        ...
        for action in self.all_actions:
            # Create PgNode_a objects for the action
            node_a = PgNode_a(action)
            # Check if the action node is reachable by checking its preconditions
            if node_a.prenodes.issubset(self.s_levels[level]):
                ...
                for node_s in self.s_levels[level]:
                    ...
                    # Append the new action level to the existing
    ...
    def add_literal_level(self, level):
        ...
        for node_a in self.a_levels[level-1]:
            # Get nodes effected by the preceding action and add the nodes to the level's state nodes
            for node_s in node_a.effnodes:
                # Adds the effect node to the set of state nodes
                s_nodes.add(node_s)
                # Makes proper connections to the nodes
                node_s.parents.add(node_a)
                node_a.children.add(node_s)
        # Append the new state level to the existing
    ...
    def inconsistent_effects_mutex(self, node_a1: PgNode_a, node_a2: PgNode_a) -> bool:
        ...
        # Check if node_a1 actions are canceled by node_a2 actions
        for effect in node_a1.action.effect_add:
            if effect in node_a2.action.effect_rem:
                return True

        # Check if node_a2 actions are canceled by node_a1 actions
    ...
    def interference_mutex(self, node_a1: PgNode_a, node_a2: PgNode_a) -> bool:
        ...
        # Check if node_a1 (+) effects conflict with node_a2 (-) preconditions
        for effect in node_a1.action.effect_add:
            if effect in node_a2.action.precond_neg:
                return True

        # Check if node_a2 (+) effects conflict with node_a1 (-) preconditions
        for effect in node_a2.action.effect_add:
    ...
    def competing_needs_mutex(self, node_a1: PgNode_a, node_a2: PgNode_a) -> bool:
        ...
        # Check if node_a1 preconditions are mutually exclusive with node_a2 preconditions
        for node_a1_precondition in node_a1.parents:
            for node_a2_precondition in node_a2.parents:
                if node_a1_precondition.is_mutex(node_a2_precondition):
                    return True
    ...
    def negation_mutex(self, node_s1: PgNode_s, node_s2: PgNode_s) -> bool:
        ...
        # Check if two state literals don't have opposite polarity
    ...
    def inconsistent_support_mutex(self, node_s1: PgNode_s, node_s2: PgNode_s):
        ...
        # Check if node_s1 preconditions are mutually exclusive with node_s2 preconditions
        for node_s1_precond in node_s1.parents:
            for node_s2_precond in node_s2.parents:
                if not node_s1_precond.is_mutex(node_s2_precond):
                    return False
